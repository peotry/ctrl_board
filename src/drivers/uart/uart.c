...
int uart_OpenDev(const char *dev_name)
{
    if(NULL == dev_name)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "Param error!");
        return -1;
    }
    ...
    fd = open(dev_name, O_RDWR | O_NOCTTY | O_NDELAY);// 读写 | 非终端 | 非阻塞
    if(fd < 0)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "%s: open error!", dev_name);
        return -1;
    }
    else
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "%s: open success!", dev_name);
        return fd;
    }
...
UARTErrCode uart_Set(int fd, int speed, int data_bits, int stop_bits, char parity)
{
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "The uart is not open!");
    ...
    //非阻塞 模式 TODO 如果需要修改 参考http://blog.csdn.net/mj5742356/article/details/7879345
    newtio.c_cc[VTIME] = 0;
    newtio.c_cc[VMIN]  = 0;
    tcflush(fd, TCIFLUSH);
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "uart set failed!");
        return WV_UART_SET_CONFIG_ERR;
    }

    LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "uart set success!");
...
/*****************************************************************************
  ...
  Input:        int fd              -       串口打开句柄
  Output:       U8 *pReadBuf        -       读出来的内容
                S32 * ps32Readlen   -       读出来的长度
  Return:       UARTErrCode
  Author:       ruibin.zhang
...
UARTErrCode uart_Read(int fd, U8 *pReadBuf, S32 * ps32Readlen)
{
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "Param is NULL!");
        return WV_UART_PARAM_ERR;
    }
    ...
    *ps32Readlen = read(fd, pReadBuf, UART_RD_LENGTH);
    ...
    if(*ps32Readlen > 0)
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "uart read len %d!", *ps32Readlen);
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "[%s]", pReadBuf);
    }
    uart_UnLock();
    return WV_UART_SUCCESS;
}
...
UARTErrCode uart_Write(int fd, U8 *pWriteBuf, S32 s32Writelen)
{
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "uart written is't match len %d/%d!", s32Writtenlen, s32Writelen);
    }
    uart_UnLock();
    return WV_UART_SUCCESS;
}
...
UARTErrCode uart0_Init()
{
    ...
    if(g_UART0Fd > 0)
    {
       LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "%s: already open!", UART0_DEV_NAME);
    ...
    if(g_UART0Fd < 0)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "%s: open error!", UART0_DEV_NAME);
        return WV_UART_OPEN_ERR;
    }
    else
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "%s: open success!", UART0_DEV_NAME);
    }
...
UARTErrCode uart1_Init()
{
    ...
    if(g_UART1Fd > 0)
    {
       LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "%s: already open!", UART1_DEV_NAME);
       //FIXME
       return WV_UART_ALREADY_OPEN_ERR;
    }
    ...
    if(g_UART1Fd < 0)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "%s: open error!", UART1_DEV_NAME);
        return WV_UART_OPEN_ERR;
    }
    else
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "%s: open success!", UART1_DEV_NAME);
    }
