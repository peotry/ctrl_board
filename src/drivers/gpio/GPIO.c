...
GPIOErrCode GPIO_CreateGPIOController(U32 u32GPIONum)
{
    ...
    if(0 == u32LinuxPin)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "GPIO index %d is error!", u32GPIONum);
    ...
    LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "Command: [%s]", u8CommandStr);
    ...
    if(NULL == pdir)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "GPIO%d create controller fail!", u32GPIONum);
...
GPIOErrCode GPIO_SetGPIODirection(U32 u32GPIONum , GPIODirectionType u32GPIODirection)
{
    ...
    if(-1 == fd)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "Can't open gpio [%d] controller [%s]!", u32LinuxPin, u8GpioControllerName);
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "GPIODirection set %d is error!", u32GPIODirection);
    ...
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "Can't write gpio [%d] controller [%s]!", u32LinuxPin, u8GpioControllerName);
        close(fd);
...
GPIOErrCode GPIO_SetGpio(U32 u32GPIONum, U32 u32GPIOValue)
{
    ...
    if(NULL == pdir)
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "GPIO%d need to creat controller!", u32GPIONum);
    ...
    {
        LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "GPIO set direction error!");
...
GPIOErrCode GPIO_GetGpio(U32 u32GPIONum,  U32 * u32GPIOValue)
{
    ...
    if(nRet != 2)
    {
        LOG_PRINTF(LOG_LEVEL_ERROR, LOG_MODULE_DRIVERS, "Can't read gpio [%d] controller [%s]!", u32LinuxPin, u8GpioControllerName);
        close(fd);
    ...
    else
    {
        * u32GPIOValue = (U32)atoi((char *)u8value);
        //LOG_PRINTF(LOG_LEVEL_DEBUG, LOG_MODULE_DRIVERS, "read gpio [%d] value: [%d]!", u32LinuxPin, *u32GPIOValue);
    }
...
/*
* function: GPIO_SetGpio2
*
* description: 设置直接使用set 0/1, 不对其方向进行设置。
*
* input:  @
*
* output: @
...
* author: linsheng.pan
*/
GPIOErrCode GPIO_SetGpio2(U32 u32GPIONum, U32 u32GPIOValue)
{
    U8  u8GpioControllerDir[MAX_GPIO_CONTTROLLER_NAME_LEN]  = {0};
    U8  u8GpioControllerName[MAX_GPIO_CONTTROLLER_NAME_LEN] = {0};
    U32 u32LinuxPin = 0;
