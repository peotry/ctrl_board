...
extern CIMAX_PCMCIA_HANDLESTRUCT_t              g_CIMax_CIHandles[2]; 
extern U8                                       g_u8MMI_CurrentMenuLevel[2];
    ...
    {                
        case CI_SLOT_EMPTY:
        {
            ...
            if (u8Slot == 0)
            {
                ...
                memcpy(&g_ptWVCIStatusInst->m_ucSlot0Name,          "EMPTY",    WVCI_SLOT_NAME_SIZE);
                memset(&g_ptWVCIStatusInst->m_ucSlot0Manufacturer,  0x00,       WVCI_SLOT_NAME_SIZE);                        
                memcpy(&g_ptWVCIStatusInst->m_ucSlot0Manufacturer,  "NULL ",    WVCI_SLOT_NAME_SIZE);
                g_ptWVCIStatusInst->m_u16SystemIDNum[u8Slot] = 0;
                for (kk = 0; kk < 32; kk++)
                {
            ...
            if (u8Slot == 1)
            {
                ...
                memcpy(&g_ptWVCIStatusInst->m_ucSlot1Name,          "EMPTY",    WVCI_SLOT_NAME_SIZE);
                memset(&g_ptWVCIStatusInst->m_ucSlot1Manufacturer,  0x00,       WVCI_SLOT_NAME_SIZE);                        
                memcpy(&g_ptWVCIStatusInst->m_ucSlot1Manufacturer,  "NULL ",    WVCI_SLOT_NAME_SIZE);
                g_ptWVCIStatusInst->m_u16SystemIDNum[u8Slot] = 0;
                for (kk = 0; kk < 32; kk++)
                {
                    g_ptWVCIStatusInst->m_u16SystemIDList[u8Slot][kk] = 0;
        ...
        case CI_SLOT_MODULE_INSERTED:
        {
            ...
            {
                memset(&g_ptWVCIStatusInst->m_ucSlot0Name, 0x00, WVCI_SLOT_NAME_SIZE);                        
                memcpy(&g_ptWVCIStatusInst->m_ucSlot0Name, "Initializing...", WVCI_SLOT_NAME_SIZE);
            }
            if (u8Slot == 1)
            {
                memset(&g_ptWVCIStatusInst->m_ucSlot1Name, 0x00, WVCI_SLOT_NAME_SIZE);
                memcpy(&g_ptWVCIStatusInst->m_ucSlot1Name, "Initializing...", WVCI_SLOT_NAME_SIZE);
            }
            #ifdef DEBUG_ENABLE_UI_INFO
        ...
        {            
            if (u8Slot == 0)
            {
                memset(g_ptWVCIStatusInst->m_ucSlot0Name, 0, WVCI_SLOT_NAME_SIZE);
                memcpy(g_ptWVCIStatusInst->m_ucSlot0Name,          &CiSlot[u8Slot].pszModuleName, (WVCI_SLOT_NAME_SIZE-1));
                memcpy(g_ptWVCIStatusInst->m_ucSlot0Manufacturer,  (char *)g_CIMax_CIHandles[u8Slot].tPCMCIAInfo.pszManufacturerName, (WVCI_SLOT_NAME_SIZE-1));
                
            }
            if (u8Slot == 1)
            {
                memset(g_ptWVCIStatusInst->m_ucSlot1Name, 0, WVCI_SLOT_NAME_SIZE);
                memcpy(g_ptWVCIStatusInst->m_ucSlot1Name, &CiSlot[u8Slot].pszModuleName, (WVCI_SLOT_NAME_SIZE - 1));
                memcpy(g_ptWVCIStatusInst->m_ucSlot1Manufacturer,  (char *)g_CIMax_CIHandles[u8Slot].tPCMCIAInfo.pszManufacturerName, (WVCI_SLOT_NAME_SIZE-1));
            }
